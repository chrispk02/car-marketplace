...
// --- 1. MODAL BROKER FORM (TRÁI TIM CỦA HỆ THỐNG) ---
// Form này thu thập Data khách hàng (Lead) để Admin xử lý
const LeadFormModal = ({ car, onClose }: { car: any; onClose: () => void }) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      // Gửi về endpoint Backend NestJS của bạn
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
      <div className="bg-white rounded-[2.5rem] p-10 max-w-md w-full shadow-2xl relative animate-in fade-in zoom-in duration-300 border border-gray-100">
        <button onClick={onClose} className="absolute top-6 right-8 text-gray-300 hover:text-primary text-3xl font-light">×</button>
        ...
        {success ? (
          <div className="text-center py-10">
            ...
            <p className="text-gray-500 font-medium leading-relaxed">Admin sẽ liên hệ báo giá chiếc <br/><span className="text-primary font-bold">{car.title}</span></p>
            <button onClick={onClose} className="mt-8 w-full py-4 bg-primary text-white font-bold rounded-2xl shadow-xl hover:brightness-110 transition-all">HOÀN TẤT</button>
          </div>
        ) : (
          <>
            <h3 className="text-2xl font-black text-primary mb-1 uppercase tracking-tighter italic">Precision Brokerage</h3>
            <p className="text-gray-400 mb-8 text-[11px] font-black uppercase tracking-widest">Đăng ký tư vấn xe: {car.title}</p>
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              <div className="space-y-1">
                <label className="text-[10px] font-black text-gray-400 uppercase ml-2">Họ và tên</label>
                <input name="name" required className="w-full p-4 bg-gray-50 border border-gray-100 rounded-2xl outline-none font-bold text-sm focus:ring-2 focus:ring-primary transition-all" />
              </div>
              <div className="space-y-1">
                <label className="text-[10px] font-black text-gray-400 uppercase ml-2">Số điện thoại</label>
                <input name="phone" required className="w-full p-4 bg-gray-50 border border-gray-100 rounded-2xl outline-none font-bold text-sm focus:ring-2 focus:ring-primary transition-all" />
              </div>
              <div className="space-y-1">
                <label className="text-[10px] font-black text-gray-400 uppercase ml-2">Ngân sách dự kiến (VNĐ)</label>
                <input name="budget" type="number" className="w-full p-4 bg-gray-50 border border-gray-100 rounded-2xl outline-none font-bold text-sm focus:ring-2 focus:ring-primary transition-all" />
              </div>
              <button disabled={loading} className="primary-gradient text-white font-black py-5 rounded-2xl shadow-xl hover:brightness-110 active:scale-95 transition-all mt-6 uppercase tracking-[0.2em] text-[10px]">
                {loading ? 'ĐANG XỬ LÝ...' : 'GỬI YÊU CẦU CHO ADMIN'}
              </button>
            </form>
          </>
        )}
...
// --- 2. CÁC SECTION UI (GIỮ NGUYÊN CSS SANG TRỌNG CỦA BẠN) ---
  ...
  <nav className="fixed top-0 w-full z-50 bg-white/95 backdrop-blur-md border-b border-gray-100 transition-all duration-300">
    <div className="flex justify-between items-center px-6 lg:px-12 py-4 max-w-[1440px] mx-auto w-full">
      <div className="flex items-center gap-12">
        <a className="text-2xl font-black italic tracking-tighter text-primary" href="/">Precision Curator</a>
      ...
      <div className="flex items-center gap-4">
        <button onClick={() => window.location.href='/login'} className="px-6 py-2.5 rounded-xl font-bold text-sm transition-all duration-300 text-primary hover:bg-primary/5 border border-primary/10">
          Sign In/Join
        </button>
      </div>
    </div>
  ...
  return (
    <section className="relative h-[850px] w-full flex items-center justify-center px-6 overflow-hidden pt-16">
      ...
      <div className="relative z-10 w-full max-w-5xl text-center">
        <h1 className="text-white text-6xl md:text-8xl font-black tracking-tighter mb-8 leading-none italic uppercase">FIND YOUR <span className="text-purple-300">MOMENTUM</span></h1>
        <div className="bg-white rounded-[2.5rem] shadow-[0_32px_64px_-12px_rgba(0,0,0,0.2)] overflow-hidden text-left max-w-5xl mx-auto">
          <div className="flex bg-gray-50/50 border-b">
            <button className="flex-1 py-7 text-xs font-black border-b-4 border-primary text-primary uppercase tracking-[0.2em]">Shop Buy</button>
            <button className="flex-1 py-7 text-xs font-black text-gray-400 uppercase tracking-[0.2em] hover:bg-gray-100 transition-colors">Sell</button>
            <button className="flex-1 py-7 text-xs font-black text-gray-400 uppercase tracking-[0.2em] hover:bg-gray-100 transition-colors">Service</button>
          </div>
...
const FeaturedCurations = ({ onSelectCar }: { onSelectCar: (car: any) => void }) => {
  ...
  useEffect(() => {
    // Dữ liệu mẫu (Khi Backend chạy thật, axios sẽ lấy từ DB)
    setCars([
    ...
    <section className="bg-white py-32 px-6">
      <div className="max-w-[1440px] mx-auto">
        <div className="flex justify-between items-end mb-16">
          ...
          <a className="text-xs font-black text-secondary flex items-center gap-3 group transition-all uppercase tracking-widest" href="#">
            ...
            <div key={car.id} className="bg-white rounded-[2rem] overflow-hidden shadow-[0_8px_30px_rgb(0,0,0,0.04)] group hover:shadow-[0_32px_64px_rgba(0,0,0,0.08)] transition-all duration-700 border border-gray-50 flex flex-col">
              ...
              <div className="p-8 flex-1 flex flex-col">
                ...
                <div className="flex items-center justify-between mt-auto pt-8 border-t border-gray-100">
                  ...
                  {/* NÚT MUA: Kích hoạt Modal Broker */}
...
// --- 3. MAIN APP (CẤU TRÚC ĐÃ HOÀN THIỆN) ---
...
export default function App() {
  ...
  const handleSearch = (f: any) => {
    ...
    // Khi click nút search, logic sẽ gọi API tại đây
  };
  ...
  return (
    <div className="min-h-screen bg-white text-on-surface font-sans selection:bg-primary selection:text-white">
      ...
      <main>
        ...
        {/* Browse Section (Giữ nguyên UI sang trọng của bạn) */}
        <section className="py-32 px-6 max-w-[1440px] mx-auto overflow-hidden">
          ...
          <div className="flex overflow-x-auto gap-8 pb-10 no-scrollbar scroll-smooth">
            {['SUV', 'Sedan', 'Pickup', 'Coupe', 'Hatchback', 'Luxury'].map((label, idx) => (
              <a key={idx} className="flex-shrink-0 group flex flex-col items-center gap-8 p-14 rounded-[3rem] bg-white hover:bg-primary transition-all duration-700 w-64 shadow-[0_8px_30px_rgb(0,0,0,0.02)] border border-gray-50" href="#">
                <span className="material-symbols-outlined text-6xl text-primary group-hover:text-white transition-all scale-100 group-hover:scale-125 duration-700">directions_car</span>
        ...
        <FeaturedCurations onSelectCar={(car) => setSelectedCar(car)} />

        {/* Marketing Banners (Đã gắn link logic) */}
        <section className="py-32 px-6 max-w-[1440px] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12">
          <div className="relative overflow-hidden rounded-[4rem] bg-primary p-16 text-white group flex flex-col md:flex-row items-center gap-12 shadow-2xl">
            <div className="relative z-10 flex-1">
              ...
              <button onClick={() => window.location.href='/signup'} className="bg-white text-primary font-black px-14 py-6 rounded-2xl shadow-2xl uppercase tracking-widest text-[10px] hover:scale-105 transition-transform">Get Started</button>
            </div>
            <span className="material-symbols-outlined text-[240px] text-white/10 rotate-12 group-hover:scale-110 transition-all duration-1000">sell</span>
          </div>
          <div className="relative overflow-hidden rounded-[4rem] bg-white border border-gray-100 p-16 group flex flex-col md:flex-row items-center gap-12 shadow-[0_32px_64px_rgba(0,0,0,0.04)] hover:shadow-2xl transition-all duration-700">
            ...
            <span className="material-symbols-outlined text-[240px] text-primary/5 -rotate-12 group-hover:scale-110 transition-all duration-1000">credit_score</span>
          </div>
        </section>
      </main>

      <footer className="bg-white border-t border-gray-100 w-full pt-32 pb-16 px-6">
        <div className="max-w-[1440px] mx-auto text-center md:text-left flex flex-col items-center md:items-start gap-12">
          <a className="text-4xl font-black italic tracking-tighter text-primary" href="#">Precision Curator</a>
          <p className="text-gray-300 text-[10px] uppercase tracking-[0.5em] font-black">© 2026 Precision Curator Automotive. All rights reserved.</p>
        </div>
      </footer>

      {/* MODAL BROKER FORM: Hiển thị khi khách hàng click nút Buy */}
      {selectedCar && (
        <LeadFormModal 
          car={selectedCar} 
